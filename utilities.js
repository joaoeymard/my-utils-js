    ...
    case "semana":
      return [
        new Date(
          agora.getFullYear(),
          agora.getMonth(),
          agora.getDate() - cod_semana
        ).getTime(),
        new Date(
          agora.getFullYear(),
          agora.getMonth(),
          agora.getDate() + (6 - cod_semana)
        ).getTime(),
      ];
    case "semanapassada":
      return [
        new Date(
          agora.getFullYear(),
          agora.getMonth(),
          agora.getDate() - (cod_semana + 7)
        ).getTime(),
        new Date(
          agora.getFullYear(),
          agora.getMonth(),
          agora.getDate() - (cod_semana + 1)
        ).getTime(),
      ];
    case "mes":
      return [
...
function preview_age(strEl) {
  document.querySelectorAll(strEl).forEach((e) => {
    e.onblur = function (ev) {
      const age = format_period(
        Date.now() - Date.parse(ev.target.value + "T00:00:00"),
        "years"
      );

      ev.target.nextElementSibling.lastElementChild.textContent =
